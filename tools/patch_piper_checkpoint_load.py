...
def _install_trusted_checkpoint_loader() -> None:
    import lightning_fabric.plugins.io.torch_io as torch_io
    import lightning_fabric.utilities.cloud_io as cloud_io

    def _trusted_pl_load(path_or_url, map_location=None):
    ...
    cloud_io._load = _trusted_pl_load
    torch_io.pl_load = _trusted_pl_load
...
def main():
    ...
    if ckpt_path:
        _LOGGER.info("Installing trusted checkpoint loader for: %s", ckpt_path)
        _install_trusted_checkpoint_loader()
...
def main() -> int:
    if len(sys.argv) != 2:
        print('Usage: patch_piper_checkpoint_load.py <path-to-__main__.py>', file=sys.stderr)
        return 1
